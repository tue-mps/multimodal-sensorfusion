...
class Detection_Header(nn.Module):
    ...
    def __init__(self, config, use_bn=True, reg_layer=2, input_angle_size=0):
...
class FFTRadNet(nn.Module):
    def __init__(self, mimo_layer, channels, blocks, detection_head, segmentation_head, config, regression_layer=2):
        ...
        if (self.detection_head):
            self.detection_header = Detection_Header(config=config, input_angle_size=channels[3] * 4, reg_layer=regression_layer)
