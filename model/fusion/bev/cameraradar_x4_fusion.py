...
class FPN_BackBone_radar(nn.Module):
    ...
    def __init__(self, num_block, channels, channels_bev, block_expansion, mimo_layer, use_bn=True):
        ...
        self.cameraenc = PolarSegFusionNet_BEV(channels_bev=channels_bev, blocks=num_block)
    ...
    def forward(self, x, bev_inputs):
        ...
        features_camera = self.cameraenc(bev_inputs)
        ...
        features_radar['x4_fused'] = torch.cat((features_radar['x4'], features_camera['x4']), axis=1)

        features_radar['x3_fused'] = torch.cat((features_radar['x3'], features_camera['x3']), axis=1)

        features_radar['x2_fused'] = torch.cat((features_radar['x2'], features_camera['x2']), axis=1)
...
class RangeAngle_Decoder(nn.Module):
    def __init__(self, ):
        ...
        self.L3 = nn.Conv2d(448, 224, kernel_size=1, stride=1, padding=0)
        self.L2 = nn.Conv2d(288, 224, kernel_size=1, stride=1, padding=0)
        self.L1 = nn.Conv2d(736, 224, kernel_size=1, stride=1, padding=0)

    def forward(self, features_radar):
        T4 = self.L1(features_radar['x4_fused']).transpose(1, 3)
        T3 = self.L3(features_radar['x3_fused']).transpose(1, 3)
        T2 = self.L2(features_radar['x2_fused']).transpose(1, 3)
...
class PolarSegFusionNet_RA(nn.Module):
    def __init__(self, mimo_layer, channels, channels_bev, blocks):
        super(PolarSegFusionNet_RA, self).__init__()

        self.FPN = FPN_BackBone_radar(num_block=blocks, channels=channels,
                                      channels_bev=channels_bev,
                                      block_expansion=4, mimo_layer=mimo_layer,
                                      use_bn=True)

    def forward(self, x, channels_bev):
        features_radar = self.FPN(x, channels_bev)
        return features_radar
...
class FPN_BackBone_camera(nn.Module):
    ...
    def forward(self, x):
        ...
        features_camera['x0'] = x
        features_camera['x1'] = x1
        features_camera['x2'] = x2
        features_camera['x3'] = x3
        features_camera['x4'] = x4

        return features_camera
...
class cameraradar_fusion_x4_bev(nn.Module):
    def __init__(self, mimo_layer, channels, channels_bev, blocks, detection_head,segmentation_head,config,regression_layer=2):
        super(cameraradar_fusion_x4_bev, self).__init__()

        self.detection_head = detection_head
        self.segmentation_head = segmentation_head

        self.radarenc = PolarSegFusionNet_RA(mimo_layer=mimo_layer, channels=channels, channels_bev=channels_bev, blocks=blocks)
    ...
    def forward(self, bev_inputs, ra_inputs):
        ...
        features_radar = self.radarenc(ra_inputs, bev_inputs)
...
#     #fusion
#
#     net = cameraradar_fusion_x4_bev(mimo_layer=mimo_layer, channels=channels,
#                                    channels_bev=channels_bev, blocks=blocks,
