...
import json
import argparse
# our camera based architecture has been used for early fusion
# camera+radar, camera+lidar, camera+radar+lidar can be achieved with this architecture
...
class FPN_BackBone_camera(nn.Module):
    ...
    def __init__(self, num_block, channels, config, block_expansion, use_bn=True):
        super(FPN_BackBone_camera, self).__init__()
        self.block_expansion = block_expansion
        self.use_bn = use_bn

        if config['model']['view_perspective'] == 'True' and config['architecture']['perspective']['early_fusion'] == 'True':
            if config['model']['camera_input']=='True' and config['model']['radar_input']=='True' and config['model']['lidar_input'] == 'True':
                in_ch = 5
            else:
                in_ch = 4

        self.warmup = warmupblock(in_ch, 32, kernel_size=3, use_bn=True)
        self.in_planes = 32
...
class encoderdecoder(nn.Module):
    def __init__(self, channels_bev, blocks,config):

        super(encoderdecoder, self).__init__()

        self.FPN = FPN_BackBone_camera(num_block=blocks, channels=channels_bev, config=config, block_expansion=4, use_bn=True)
...
class earlyfusion(nn.Module):
    def __init__(self, channels_bev, blocks, detection_head,segmentation_head,config):
        ...
        #####################################################
        self.cameraencdec = encoderdecoder(channels_bev=channels_bev, blocks=blocks,config=config)
    ...
    def forward(self, earlyconcat):

        #####################################################
        ########### DETECTION WITH CAMERA INPUT ##########
        #####################################################
        out = {'Detection': [], 'Segmentation': []}
        decoder_output = self.cameraencdec(earlyconcat)
...
#     parser.add_argument('-c', '--config', default='/media/kach271771/LocalDisk/PhD/VM_data/RADIal_experiments/05_MultiTask_Fusion/config/config_allmodality.json',type=str,
...
#
#     camera_input = torch.randn((2, 4, 540, 960))
#
#     net = earlyfusion(channels_bev=[16,32,64,128], blocks=[3, 6, 6, 3],
#                       detection_head=True,segmentation_head=True,config=config)
#
