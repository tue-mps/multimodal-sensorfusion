...
class FPN_BackBone_camera(nn.Module):
    ...
    def forward(self, x, features_lidar):
        x = self.warmup(x)
        x = self.conv(x)
        x = self.bn(x)
        x = self.relu(x)
        ...
        lidar_feature_x2 = F.interpolate(features_lidar['x2'], (135, 240))
        lidar_feature_x3 = F.interpolate(features_lidar['x3'], (68, 120))
        lidar_feature_x4 = F.interpolate(features_lidar['x4'], (34, 60))

        features['x4_fused'] = torch.cat((features['x4'], lidar_feature_x4), axis=1)
        features['x3_fused'] = torch.cat((features['x3'], lidar_feature_x3), axis=1)
        features['x2_fused'] = torch.cat((features['x2'], lidar_feature_x2), axis=1)
...
class cameraonly_perspective(nn.Module):
    ...
    def forward(self, x, lidar_feature_x4):
        features = self.FPN(x, lidar_feature_x4)
...
class fftradnet_adapted(nn.Module):
    ...
    def forward(self, cam_inputs, features_lidar):

        out = {'Detection': [], 'Segmentation': []}
        decoder_output = self.cameraencdec(cam_inputs, features_lidar)
...
class PIXOR(nn.Module):
    ...
    def forward(self, x):
        ...
        features_lidar['x0'] = x
        features_lidar['x1'] = x_1
        features_lidar['x2'] = x_2
        features_lidar['x3'] = x_3
        features_lidar['x4'] = x_4

        return features_lidar #this is the output from encoder block

################## FUSION x4 ####################
class cameralidar_fusion_x4(nn.Module):
    def __init__(self, channels_bev, blocks,detection_head,segmentation_head,config):
        super(cameralidar_fusion_x4, self).__init__()

        self.lidaronly = PIXOR()
        self.cameraonly = fftradnet_adapted(channels_bev, blocks,detection_head,segmentation_head,config)

    def forward(self, cam_inputs, lidar_inputs):

        features_lidar = self.lidaronly(lidar_inputs)
        out = self.cameraonly(cam_inputs, features_lidar)

        return out

# if __name__ == "__main__":
#     camera_input = torch.randn((2, 3, 540, 960))
#     lidar_input = torch.randn((2, 46, 1030, 800))
#
#     net = cameralidar_fusion_x4(channels_bev=[16, 32, 64, 128],
#                                 blocks=[3, 6, 6, 3],
#                                 detection_head=True,segmentation_head=True)
#
#
#     fusion_afterdecoder = net(camera_input, lidar_input)
